...
export default function App() {
  ...
  const sendMessage = async (contentOverride = null, parentOverride = null) => {
    ...
    if (!isRetry) newMsgs[userMsgId] = { id: userMsgId, parent_id: parentId, role: 'user', content };
    newMsgs[botMsgId] = { id: botMsgId, parent_id: isRetry ? parentId : userMsgId, role: 'assistant', content: '' };
    ...
    await fetch('/api/chat', {
      ...
      headers: { 'Content-Type': 'application/json', 'x-db-token': settings.dbToken },
      body: JSON.stringify({
        messages: path, userMsgId: isRetry ? null : userMsgId, botMsgId,
        parentId, conversationId: convId, apiKey: settings.apiKey, model: settings.model
      })
    });
  ...
  const handleBranch = async (msgId) => {
    ...
    for (const m of path) {
      ...
      idMap[m.id] = newId;
      newMessages.push({
        id: newId, conversation_id: newConvId, parent_id: m.parent_id ? idMap[m.parent_id] : null,
        role: m.role, content: m.content, created_at: time++
      });
      lastNewId = newId;
    }
  ...
  const deleteMessage = async (msgId) => {
    ...
    for (let i = 0; i < toDelete.length; i++) {
      ...
      Object.values(newMsgs).forEach(m => {
        if (m.parent_id === id && !toDelete.includes(m.id)) toDelete.push(m.id);
      });
    }
    toDelete.forEach(id => delete newMsgs[id]);
    setMessages(newMsgs);
    if (toDelete.includes(currentId)) setCurrentId(messages[msgId].parent_id || null);
  };

  const handleRetry = (msgId) => sendMessage(messages[msgId].content, messages[msgId].parent_id);
  ...
  const switchBranch = (siblingId) => {
    let leaf = siblingId, found = true;
    while(found) {
      const child = Object.values(messages).find(m => m.parent_id === leaf);
      if (child) leaf = child.id; else found = false;
    }
    setCurrentId(leaf);
  };
  ...
  return (
    ...
    <Container fluid className="p-0 overflow-hidden d-flex" style={{ height: '100dvh' }}>
      ...
      <SettingsModal 
        show={showSettings} onHide={() => setShowSettings(false)} 
        settings={settings} setSettings={setSettings} onSave={saveSettings} 
      />
      ...
      <div className="d-flex flex-column bg-white h-100 flex-grow-1 position-relative">
        ...
        <div className="d-md-none p-2 border-bottom d-flex align-items-center bg-light">
          ...
          <span className="ms-3 fw-bold text-truncate">
          ...
          {activePath.length === 0 ? (
            ...
            <Container className="px-0" style={{ maxWidth: '800px' }}>
              {activePath.map(msg => {
                ...
                return (
                  <MessageNode 
                    key={msg.id} msg={msg} siblings={siblings} index={index}
                    switchBranch={switchBranch} handleCopy={handleCopy} handleBranch={handleBranch}
                    handleRetry={handleRetry} deleteMessage={deleteMessage} modelName={settings.model}
                  />
                );
              })}
