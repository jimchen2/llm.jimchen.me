...
export async function GET(req) {
  ...
  // Fetch a single conversation (used to restore its mode)
  const singleId = url.searchParams.get('id');
  if (singleId) {
    const data = await redis.hgetall(`conv:${singleId}`);
    if (!data || !data.id) return NextResponse.json({});
    return NextResponse.json({ ...data, mode: normalizeMode(data.mode) });
  }
...
export async function POST(req) {
  ...
  pipeline.hset(`conv:${id}`, {
    ...
    mode: normalizeMode(mode), // 'tech' by default
    created_at: now,
  });
...
export async function PUT(req) {
  ...
  await redis.hset(`conv:${id}`, { mode: normalizeMode(mode) });
  await redis.expire(`conv:${id}`, CACHE_TTL_SECONDS);
  return NextResponse.json({ success: true });
}
